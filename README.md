...
## このアプリは、本の紹介を中心として、本の情報を共有することに特化しています。ジャンルをクリックして、そのジャンルの本の一覧をチェックしたり、クレジット決済をすればチャットルームに入室して他のユーザーとよりディープな話題で盛り上がることができます。(以下がherokuでデプロイされたアプリのURLになります)
...
## チャットルーム入室時のクレジット決済機能、複数でのチャット機能確認用アカウント
## password (最低6文字、全角使用不可)
## email ~~@gmail.com
## user_name （最高８文字、全角使用不可）
...
## ルーム通知機能
## 他のユーザーからルームに招かれた場合(自分が作成したルームじゃないルームが立ち上がった場合）ルーム名の横に"招待されました"と表示されることで、誰かにルームに招かれたことを確認できます
...
## ローカルでの操作方法
## まず、ブラウザの検索窓にlocalhost3000と打ち込みます。そのあと、header部分の新規登録からユーザーネームとパスワード、emailを打ち込んでください。（ユーザー名は最大８文字、パスワードは最低６文字、全角は使用不可です）
